...
module "postgres_psc_data" {
  ...
  network_url     = "projects/${var.data_project_id}/global/networks/${var.data_psc_forwarding_network}"
...
module "postgres_psc_app" {
  ...
  network_url     = "projects/${var.project_id}/global/networks/${var.app_psc_forwarding_network}"
  dns_record_name = module.postgres_data.db_dns_name
  depends_on = [
    time_sleep.wait_for_apis
  ]
